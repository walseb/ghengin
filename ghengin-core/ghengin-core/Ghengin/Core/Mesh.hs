...
createMesh (RenderPipeline gpip rpass (rdset, rres, _bmap, dpool0) shaders uq) props0 (SV.fromList -> vs) = enterD "createMesh" Linear.do
  ...
  pure ( mkMesh (SimpleMesh vertexBuffer (dset0, rmap0) uniq) props1
       , RenderPipeline gpip rpass (rdset, rres, _bmap, dpool1) shaders uq
       )
...
createMeshWithIxs (RenderPipeline gpip rpass (rdset, rres, _bmap, dpool0) shaders uq) props0 (SV.fromList -> vertices) (SV.fromList -> ixs) = enterD "createMeshWithIxs" Linear.do
  Ur uniq      <- liftSystemIOU newUnique
