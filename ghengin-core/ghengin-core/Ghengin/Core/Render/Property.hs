...
-- also line this up).
  ...
  Texture2DBinding :: Alias (Texture2D fmt coordTy texelTy usage) ⊸ PropertyBinding (Texture2D fmt coordTy texelTy usage)
...
type family PBInv α = r | r -> α where
  PBInv (Texture2D fmt coordTy texelTy usage) = Alias (Texture2D fmt coordTy texelTy usage)
  PBInv x               = Ur x
...
instance (Unsatisfiable (Text "The requested property " :<>: ShowType α :<>: Text " does not exist in the properties list: " :<>: ShowType β)) => HasPropertyAt' n n' φ '[] α where
...
editProperty prop update i dset resmap0 = Linear.do
  ...
  where
    ...
    -- ROMES:TODO: Textures!!!!
    updateTextureBinding :: forall fmt coordTy texelTy usage. DescriptorSet ⊸ Alias (Texture2D fmt coordTy texelTy usage) ⊸ Renderer DescriptorSet
