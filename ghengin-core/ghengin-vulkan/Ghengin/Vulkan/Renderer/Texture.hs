...
type Texture2D :: ImageFormat Nat -> ImageCoordinateKind -> Type -> ImageUsage -> Type
data Texture2D (fmt :: ImageFormat Nat) (coordTy :: ImageCoordinateKind) texelTy (usage :: ImageUsage)
...
texture :: FilePath -> Alias Sampler ⊸ Renderer (Alias (Texture2D (RGBA8 UNorm) FloatingPointCoordinates Float Sampled))
...
freeTexture :: Texture2D fmt coordTy texelTy usage ⊸ Renderer ()
...
textureFromDynamicImage :: DynamicImage
                        -> Alias Sampler
                         ⊸ Renderer (Alias (Texture2D (RGBA8 UNorm) FloatingPointCoordinates Float Sampled))
...
newTexture :: (Pixel px, Typeable px, CompatiblePixel px fmt)
           ...
           -> Alias Sampler
            ⊸ Renderer (Alias (Texture2D fmt coordTy texelTy usage))
...
instance Shader.ShaderData (Texture2D fmt coordTy texelTy usage) where
  type FirType (Texture2D fmt coordTy texelTy usage) =
        FIR.Image (FIR.Properties coordTy texelTy FIR.TwoD (Just FIR.NotDepthImage) FIR.NonArrayed FIR.SingleSampled usage (Just fmt))
